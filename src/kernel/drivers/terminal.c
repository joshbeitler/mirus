...
void terminal_setcolor(TerminalColor fg) { terminal.current_fg_color = fg; }
...
#define MAX_TERM_SIZE 28800
	...
	ssfn_dst.x = terminal.cursor_x; // When we move the cursor later, this
									// multiplies by character width
	ssfn_dst.y = terminal.cursor_y; // When we move the cursor later, this
									// multiplies by character width
...
void terminal_render() {
	...
	for (int y = 0; y < ssfn_dst.h; y++) {
		for (int x = 0; x < ssfn_dst.w; x++) {
			((uint32_t *)ssfn_dst.ptr)[y * (ssfn_dst.p / 4) + x] =
				0; // Set to black
		}
	}
 ...
 */
void putchar_(char c) { terminal_write_char(c); }
...
void terminal_scroll() {
	...
	for (int y = 0; y < terminal.height - 1; y++) {
		for (int x = 0; x < terminal.width; x++) {
			terminal.buffer[y * terminal.width + x] =
				terminal.buffer[(y + 1) * terminal.width + x];
		}
	}
